...
void queue_put(queue_t *q, uint8_t *buffer, size_t size)
{
    ...
    memcpy_avx_16(&q->buffer[q->tail], &m);
    memcpy_avx_16(&q->buffer[q->tail + sizeof(message_t)], buffer);
...
size_t queue_get(queue_t *q, uint8_t *buffer, size_t max)
{
    ...
    for (;;) {
        ...
        memcpy_avx_16(&m, &q->buffer[q->head]);
    ...
    //memcpy_avx_16(buffer, &q->buffer[q->head + sizeof(message_t)]);
